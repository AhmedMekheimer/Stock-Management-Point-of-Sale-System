    ...
    public class SalesInvoicesController : Controller
    {
        ...
        public async Task<IActionResult> Index(SalesInvoicesVM vm)
        {
            ...
            var invoices = await _UnitOfWork.SalesInvoices.GetAsyncIncludes(
                condition: s =>
                (string.IsNullOrEmpty(vm.Search) || s.Code.Contains(vm.Search)
                || s.ApplicationUser.UserName!.Contains(vm.Search) || s.RetailCustomer.Name.Contains(vm.Search))
            ...
            var totalPages = (int)Math.Ceiling(invoices.Count / (double)itemsInPage);
